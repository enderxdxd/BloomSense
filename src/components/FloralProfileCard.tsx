...
interface FloralProfileCardProps {
  ...
  occasion: QuizInput["occasion"] | null;
}

const SCENE_LABELS: Record<QuizInput["occasion"], string> = {
  wedding: "Your wedding, imagined",
  anniversary: "Your anniversary table, imagined",
  birthday: "Your celebration, imagined",
  sympathy: "A quiet space, imagined",
  celebration: "Your reception, imagined",
  "just-because": "In your home, imagined",
};
      ...
      <div className="px-6 pb-12 pt-10 sm:px-12 sm:pb-16 sm:pt-14">
        ...
        >
          ...
          <p className="mt-2 max-w-xl text-sm text-bloom-rose">
            Your signature flower, your palette, your style — rendered into the
            actual space you&apos;ll celebrate in.
          </p>
          <div className="mt-6">
            <SceneImage
...
function SceneImage({ status, url, label }: SceneImageProps) {
  ...
  if (status === "error") {
    return (
      <div className="flex aspect-[3/2] w-full items-center justify-center rounded-2xl border border-dashed border-bloom-rose/40 bg-bloom-cream">
        <p className="text-xs uppercase tracking-[0.32em] text-bloom-rose">
          Scene unavailable
        </p>
      </div>
